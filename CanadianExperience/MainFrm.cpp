...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_TREE_HARVEST, &CMainFrame::OnTreeHarvest)
	ON_COMMAND(ID_TREE_HARVESTTREE2, &CMainFrame::OnTreeHarvesttree2)
	ON_COMMAND(ID_SEED_SETSEED1, &CMainFrame::OnSeedSetseed1)
	ON_COMMAND(ID_SEED_SETSEED2, &CMainFrame::OnSeedSetseed2)
END_MESSAGE_MAP()
...
void CMainFrame::OnTreeHarvest()
{
	auto tree = mFactory.GetTreeImage1();
	auto harvest = tree->Harvest();
	mFactory.GetBasketImage()->AddToBasket(harvest);
	Invalidate();
}


void CMainFrame::OnTreeHarvesttree2()
{
	auto tree2 = mFactory.GetTreeImage2();
	auto harvest2 = tree2->Harvest();
	mFactory.GetBasketImage()->AddToBasket(harvest2);
	Invalidate();
}


void CMainFrame::OnSeedSetseed1()
{
	mFactory.GetTreeImage1()->ShowDialogBox();
	mPicture->UpdateObservers();
	Invalidate();
}


void CMainFrame::OnSeedSetseed2()
{
	mFactory.GetTreeImage2()->ShowDialogBox();
	mPicture->UpdateObservers();
	Invalidate();
}
